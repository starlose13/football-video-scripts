...
def generate_video_script(game_data, prediction_result, row_number):
    # Create the minimal prompt for testing Creatify API
    prompt = f"""
A brief commentary for {game_data['a_team_name']} vs {game_data['b_team_name']}. The predicted result is: {prediction_result}.
    """
    ...
    response = openai.ChatCompletion.create(
        model="gpt-4-turbo",  
        messages=[
            {"role": "system", "content": "You are an AI assistant helping to create short football commentary scripts."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=100  # Reduced to keep the script short
    )
...
def generate_ai_shorts_preview(script, aspect_ratio="9x16", style="4K realistic"):
    ...
    try:
        ...
        logging.info(f"Full response from API: {response.text}")  # Log full response
        if response.status_code == 200:
            preview_data = response.json()
...
def main():
    ...
    if match_data_list:
        # Take the first game for testing
        game_data = match_data_list[0]
        ...
        video_id = send_to_creatify(video_script)

        if video_id:
            logging.info(f"Video generation started with ID: {video_id}")
            
            # Check video status
            video_output = check_render_status(video_id)
            if video_output:
        ...
        logging.error("No game data available for processing.")

    print("First match processed successfully.")
